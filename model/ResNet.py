...
# This model is used for future pairwise ranking
...
class SiameseResNet(nn.Module):
    ...
    def __init__(
        ...
        # TODO:The input channel MIGHT be 3
        # HERE: try to use a input tensor with 11 channels
        if is_pokernet:
            self.conv1 = nn.Conv2d(
                11, self.inplanes, kernel_size=5,
                stride=1, padding=2, bias=False)
            self.bn1 = norm_layer(self.inplanes)
            self.relu = nn.ReLU(inplace=True)
            # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        ...
        # TODO: This avgpool's output size MIGHT need to modified
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # NOTE: Here the num_classes is actually the dim of feature vector
        self.fc_1 = nn.Linear(512 * block.expansion, num_classes)
        self.fc_2 = nn.Linear(num_classes, 1)
    ...
    def forward(self, x):
        if len(x.shape) == 5:
            # Into Train forward
            # input x should be kind of tensor with shape of
            # (batch, pairwise, channel, height, width)
            output_1 = self.forward_once(x[:, 0])
            output_2 = self.forward_once(x[:, 1])
            output = self.fc_2(output_1 - output_2)
            return output
        elif len(x.shape) == 4:
            # Into Eval forward (NOTE: need to be with_no_grad)
            # input x should be kind of tensor with shape of
            # (batch, channel, height, width) * 2
            feature_1 = self.forward_once(x[:, 0])
            feature_2 = self.forward_once(x[:, 1])
            output_eval_1 = self.fc_2(feature_1)
            output_eval_2 = self.fc_2(feature_2)
            return output_eval_1, output_eval_2
        else:
            raise ValueError(
                'cannot process input tensor with shape '
                'size other than 4 and 5'
                'receive tensot size of: {}'
                .format(x.shape)
            )

    # This may be deprecated
    def estimate_score(self, x):
        feature_vector = self.forward_once(x)
        score = self.fc_2(feature_vector)
        return score
...
def _siamresnet(
    ...
    if is_pokernet:
        # Currently PokerNet don't support pretrained model
        return model
    else:
        # Normal ResNet, Not the PokerNet
        if pretrained:
...
def pokersiamresnet(num_classes, pretrained=False, progress=False, **kwargs):
    r""" Siam-ResNet for PokerNet
    Using 2 ResNets as the backbone and a structure of Siamese Neural Network
    to predict a rank list for the next playing-out combs of the player

    Args:
        pretrained (bool): If True, returns a model pre-trained on Poker
        progress (bool): If True, displays a progress bar of the download
    Return:
        SiameseResNet (nn.Module)
    """
    return _siamresnet(
        'pokersiamresnet', BasicBlock, [1, 1, 1, 2], num_classes,
        pretrained, progress, True, **kwargs
    )
